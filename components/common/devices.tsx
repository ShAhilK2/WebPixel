...
import { getDeviceAnalytics } from "@/action/analytics";
...
const Devices = ({ dateRange }: { dateRange?: DateRange }) => {
  const params = useParams();
  const websiteId = params.websiteId as string;
  const [activeTab, setActiveTab] = useState<TabType>("size");

  const { data, isPending } = useQuery({
    queryKey: ["device", websiteId, dateRange?.from?.toISOString(), dateRange?.to?.toISOString()],
    queryFn: async () => {
