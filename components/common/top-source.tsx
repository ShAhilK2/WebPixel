...
import { getSourceAnalytics } from "@/action/analytics";
...
const TopSources = ({ dateRange }: { dateRange?: DateRange }) => {
  const params = useParams();
  const websiteId = params.websiteId as string;

  const { data, isPending } = useQuery({
    queryKey: ["top-sources", websiteId, dateRange?.from?.toISOString(), dateRange?.to?.toISOString()],
    queryFn: async () => {
  ...
  return (
    <Card className="py-4 px-6 gap-2 min-h-[300px]">
      <CardHeader className="p-0! pt-2!">
        <CardTitle className="text-sm font-semibold">Top Source</CardTitle>
      </CardHeader>
