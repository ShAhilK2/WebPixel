...
export async function getAnalytics(websiteId: string, from?: Date, to?: Date) {
  const { insforge, user } = await getAuthenticatedClient();
  if (!user) return { error: "Unauthorized" };

  const startISO = formatISO(startOfDay(from ?? subDays(new Date(), 29)));
  ...
  const chartData = Object.values(dailyStats).map((dayStat: any) => ({
    date: dayStat.date,
    uniqueVisitors: dayStat.visitorIds.size,
    totalPageviews: dayStat.pageviews,
    averageActiveTime:
      dayStat.activeCount > 0
...
export async function getLocationAnalytics(
  ...
  sessions?.forEach((session: any) => {
    ...
    if (!countryMap[session.country].country_code && session.country_code) {
      countryMap[session.country].country_code = session.country_code;
    }

    countryMap[session.country].visitors.add(session.visitor_id);
  });
