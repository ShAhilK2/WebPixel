...
const AnalyticsChart = ({ dateRange }: { dateRange?: DateRange }) => {
  const params = useParams();
  const websiteId = params.websiteId as string;
  const [activeMetric, setActiveMetric] =
    React.useState<MetricKey>("uniqueVisitors");

  const { data, isPending } = useQuery({
    queryKey: ["analytics", websiteId],
    queryFn: async () => {
      const res = await getAnalytics(websiteId, dateRange?.from, dateRange?.to);
      return res;
    },
  });
        ...
        >
          ...
          >
            ...
            <p className="text-xs text-muted-foreground text-center">
              S tart tracking your website to see analytics here.
            </p>
          </div>
        </div>
      )}
        ...
        {metrics?.map((metric, i) => (
          <div key={metric.key}>
            <button
              ...
              className={cn(
                `relative flex flex-col flex-[0.15]!
