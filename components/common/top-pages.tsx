...
import { getPageAnalytics } from "@/action/analytics";
...
const TopPages = ({ dateRange }: { dateRange?: DateRange }) => {
  const params = useParams();
  const websiteId = params.websiteId as string;
  const [activeTab, setActiveTab] = React.useState<TabType>("entry");

  const { data, isPending } = useQuery({
    queryKey: ["top-pages", websiteId, dateRange?.from?.toISOString(), dateRange?.to?.toISOString()],
    queryFn: async () => {
